...
public class Game {                             // majority of methods are private since they do not need to be referenced in other classes
    ...
    private Player lastCapturer; // Track the last player who captured cards
    ...
    private void processMove(Card playedCard, Player currentPlayer) {
        ...
        if (willCapture) {
            ...
            if (board.isPistiCapture(playedCard)) {
                System.out.println("PISTI! +10 points");
                playedCard.markPisti(); // Mark the played card as Pisti
                captured.get(0).markPisti(); // Mark the captured card as Pisti
            } else { 
                System.out.println(currentPlayer.getName() + " captured " + captured.size() + " card(s)!");}
    ...
    private void dealNewCards() {
        if (deck.remainingCards() == 24) {
            System.out.println("Reminder: The deck has 24 cards remaining!");
        }
    ...
    private void displayGame(){
        System.out.print("Board (Right is the top): ");
    ...
    private void endGame(){
        ...
        if (player.getRegularCardCount() > bot.getRegularCardCount()) {
            player.addToScore(3);
            System.out.println("Player has more cards: You gain +3 points!" );
        } else if (bot.getRegularCardCount() > player.getRegularCardCount()) {
            bot.addToScore(3);
            System.out.println("Bot has more cards: Bot gains +3 points!" );
        }

                // Display captured cards for both players
        System.out.println("\n=== Captured Cards ===");
        System.out.println("Player's captured cards:");
        for (Card card : player.getCollectedCards()) {
            System.out.print(card + "  ");
        }
        System.out.println();

        System.out.println("\nBot's captured cards:");
        for (Card card : bot.getCollectedCards()) {
            System.out.print(card + "  ");
        }
        System.out.println();
